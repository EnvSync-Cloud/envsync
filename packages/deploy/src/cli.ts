...
// - packages/deploy-cli/src/index.ts → enterprise
// Default here is enterprise only for direct `bun run src/cli.ts` in monorepo EE workflows.
if (!process.env.ENVSYNC_DEPLOY_FORCE_EDITION) {
	process.env.ENVSYNC_DEPLOY_FORCE_EDITION =
		process.env.npm_package_name === "@envsync-cloud/deploy" ? "oss" : "enterprise";
}
...
// Phase 7: duplicate local render helpers removed — use ./render (renderHelpers) only.
...
const MINIKMS_MIGRATE_SQL = [
	...
	"psql -h minikms_db -U postgres -d minikms -f /migrations/006_env_ca.sql",
	"psql -h minikms_db -U postgres -d minikms -f /migrations/007_pending_org_ca.sql",
].join(" && ");

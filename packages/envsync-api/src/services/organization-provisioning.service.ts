...
async function generateUniqueOrganizationSlug(name: string) {
	...
	throw new AppError(
		"Could not generate a unique organization slug. Try a different name.",
		409,
		"ORG_SLUG_CONFLICT",
	);
}
...
/**
 * Hosted dashboard: create a new organization for an existing IdP identity (Phase H2 naming).
 * Formerly WorkspaceProvisioningService.
 */
export class OrganizationProvisioningService {
	public static async createOrganizationForExistingIdentity(input: {
		...
		await runSaga("createOrganizationForExistingIdentity", sagaCtx, [
			{
				name: "create-org",
				execute: async (ctx) => {
				...
				execute: async (ctx) => {
					await AuditLogService.notifyAuditSystem({
						...
						details: {
							name: input.organizationName,
							slug,
							source: input.source ?? "hosted_dashboard",
						},
					});
				},
			},

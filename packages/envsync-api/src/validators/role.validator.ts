...
export const createRoleRequestSchema = z
	.object({
		...
		have_gpg_access: z.boolean().openapi({ example: false }),
		have_cert_access: z.boolean().openapi({ example: false }),
		have_audit_access: z.boolean().openapi({ example: false }),
...
export const updateRoleRequestSchema = z
	.object({
		...
		have_gpg_access: z.boolean().optional().openapi({ example: false }),
		have_cert_access: z.boolean().optional().openapi({ example: false }),
		have_audit_access: z.boolean().optional().openapi({ example: false }),
...
export const roleStatsResponseSchema = z
	.object({
		...
		gpg_access_count: z.number().openapi({ example: 1 }),
		cert_access_count: z.number().openapi({ example: 1 }),
		audit_access_count: z.number().openapi({ example: 2 }),

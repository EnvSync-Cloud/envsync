...
describe("envsync-enterprise-web package boundary (Phase 5b)", () => {
  ...
  test("source lives in package (integrations + license + sync + sso + keys pages present)", () => {
  ...
  test("nav exposes Enterprise group with license, sync, SSO, and keys", async () => {
    const mod = await import("../src/index.ts");
  ...
  test("keys scope is admin/master and requires kms", async () => {
    ...
    expect(rule).toBeTypeOf("function");
    const entitledAdmin = { role: { is_admin: true, is_master: false }, features: ["kms"] };
    const entitledEditor = { role: { is_admin: false, is_master: false }, features: ["kms"] };
    const unentitledAdmin = { role: { is_admin: true, is_master: false }, features: ["integrations"] };
    expect(rule(entitledAdmin as never)).toBe(true);
    expect(rule(entitledEditor as never)).toBe(false);
    expect(rule(unentitledAdmin as never)).toBe(false);
  ...
  test("org UI never wires platform break-glass", () => {
    const files = listFiles(srcRoot);
    const offenders = files.filter((file) => {
      const text = fs.readFileSync(file, "utf8");
      return /breakGlassDetachOrgKms|break-glass-detach|X-EnvSync-Platform-Token/.test(text);
    }).map((file) => path.relative(pkgRoot, file));

...
test.describe("route surface", () => {
	test("visits the core module routes", async ({ page }) => {
		...
		const routeChecks: Array<{ path: string; heading: RegExp | string; url?: RegExp }> = [
			...
			{ path: "/organisation", heading: /^(Organization|Organisation)( Settings)?$/i, url: /\/org\/?$/ },
			...
			{ path: "/organisation/license", heading: /License|Plan/i, url: /\/organisation\/license\/?$/ },
			...
			{ path: "/users", heading: "Users", url: /\/org\/access\/users\/?$/ },
			{ path: "/teams", heading: "Teams", url: /\/org\/access\/teams\/?$/ },
			{ path: "/roles", heading: "Roles", url: /\/org\/access\/roles\/?$/ },
